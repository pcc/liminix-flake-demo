...
rec {
  ...
  profile.gateway = {
    ...
    // (lib.optionalAttrs (attrs ? wlan5) {
      "${secrets.ssid}5" = rec {
        interface = attrs.wlan5 config;
        hw_mode = "a";
        channel = 36;
        ht_capab = "[HT40+]";
        vht_oper_chwidth = 1;
...
// (lib.optionalAttrs (attrs ? fs) {
  rootfsType = lib.mkForce attrs.fs;
})
